...
class DrawChart():
    ...
    # except:
    # print("Exception of pyqtgraphDrawChart")
...
def myData():
    import pandas as pd
    data = pd.read_csv("K line export_RB2005_5 minutes data.csv", encoding="gbk")
    data.index = data["Trading Time"]
    data.drop(["settlement price", "stock code", "stock name", "trading time", "up and down percentage", "up and down",
               "trading volume", "trading amount"], axis=1, inplace=True)
    data.rename(columns={"open price": "open", "close price": "close", "highest price": "high", "lowest price": "low"},
                inplace=True)
...
def mouseMoved(evt):
    ...
    if plot.sceneBoundingRect().contains(pos):
        ...
        if 0 < index < len(data.index):
            ...
            label.setHtml(
                "<p style='color:white'>Date: {0}</p><p style='color:white'>Opening: {1}</p><p style='color: white'>Close: {2}</p>".format(
